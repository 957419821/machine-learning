...
# Timing begin
time_start = Start()
def thread_hold_out(data, tv, LOOP_TIMES, alpha, update):
    w, b = initial_weights_and_bias()
    x_train, y_train, x_valid, y_valid = initial_data_train_valid(data, tv, 'hold_out')
    lock = Lock()
    ...
    try:
        ...
        print("the accuracy of logistic regression with hold-out: ", acc)
        print("---------------------------------------------------")
        # Timing end
        time_end = End()
        # show time used
        time_used(time_start, time_end, 'hold-out')
    finally:
        lock.release()
